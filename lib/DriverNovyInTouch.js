...
const RFSignalNovyInTouch = require('./RFSignalNovyInTouch');

module.exports = class extends RFDriver {

  static SIGNAL = RFSignalNovyInTouch;
    ...
    if (this.rfSendAction) {
      this.rfSendAction.registerRunListener(async (args) => {
        ...
        try {
          ...
          if (command.toLowerCase() === 'on' || command.toLowerCase() === 'aan' || command.toLowerCase() === 'ein') {
            await device.setCapabilityValue('onoff', true);
          } else if (command.toLowerCase() === 'off' || command.toLowerCase() === 'uit' || command.toLowerCase() === 'aus') {
  ...
  async onPair(session) {
    ...
    // 1. RF learning
    session.setHandler('start_rf_learning', async (data) => {
